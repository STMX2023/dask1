  ...
  ({
    ...
    return (
      <Pressable style={pressableStyle} onPress={onPress}>
        ...
        {showChevron && <Ionicons name="chevron-forward" size={16} style={styles.chevron} />}
      </Pressable>
    );
  },
);
...
const SettingsGroup = memo(
  ({ children, marginBottom = 24 }: { children: React.ReactNode; marginBottom?: number }) => {
    const isDark = useIsDarkMode();
    const theme = getTheme(isDark);
...
export default function TabTwoScreen() {
  ...
  const handleThemeChange = useCallback(
    ...
    [isDark, toggleDarkMode, systemColorScheme],
  );

  // Create callbacks for all settings items
  const handleWallpaper = useCallback(() => {
    console.warn('Wallpaper pressed');
  }, []);
  ...
  const handleSounds = useCallback(() => {
    console.warn('Sounds pressed');
  }, []);

  const handleFocus = useCallback(() => {
    console.warn('Focus pressed');
  }, []);

  const handleScreenTime = useCallback(() => {
    console.warn('Screen Time pressed');
  }, []);

  const handleFaceID = useCallback(() => {
    console.warn('Face ID pressed');
  }, []);

  const handleEmergencySOS = useCallback(() => {
    console.warn('Emergency SOS pressed');
  }, []);
  ...
  const handleGameCenter = useCallback(() => {
    console.warn('Game Center pressed');
  }, []);

  const handleiCloud = useCallback(() => {
    console.warn('iCloud pressed');
  }, []);

  const handleWallet = useCallback(() => {
    console.warn('Wallet pressed');
  }, []);

  const handleApps = useCallback(() => {
    console.warn('Apps pressed');
  }, []);

  const handleDeveloper = useCallback(() => {
    console.warn('Developer pressed');
  }, []);
  ...
  const styles = useMemo(
    () => ({
      ...
      title: tw.style('text-[22px] font-semibold', {
        color: theme.textPrimary,
        letterSpacing: -0.5,
      }),
      ...
      <View style={styles.scrollContainer}>
        ...
        >
          ...
          <View style={styles.themeSection}>
            <MotiText
              key={`appearance-${String(animationKey)}`}
              from={{ opacity: 0, translateX: -50 }}
              animate={{ opacity: 1, translateX: 0 }}
          ...
          {/* Settings Groups */}
          <MotiView
            key={`settings-${String(animationKey)}`}
            from={{ opacity: 0, translateY: 20 }}
            animate={{ opacity: 1, translateY: 0 }}
            transition={{
              delay: 150,
          ...
          >
            ...
            <SettingsGroup>
              ...
              />
              <SettingsItem icon="moon" iconColor="#5856D6" title="Focus" onPress={handleFocus} />
              <SettingsItem
                icon="hourglass"
                iconColor="#5856D6"
                title="Screen Time"
            ...
            <SettingsGroup marginBottom={24}>
              <SettingsItem icon="apps" iconColor="#5856D6" title="Apps" onPress={handleApps} />
              <SettingsItem
                icon="code-slash"
                iconColor="#8E8E93"
                title="Developer"
                onPress={handleDeveloper}
              />
            </SettingsGroup>
          </MotiView>
        </ScrollView>
