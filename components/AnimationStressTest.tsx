...
export const AnimationStressTest = () => {
  ...
  const handleStressTestPress = useCallback(() => {
    ...
    <View style={tw`p-4 bg-gray-100 dark:bg-gray-900 rounded-lg m-4`}>
      ...
      <View style={tw`mb-4`}>
        <Text style={tw`text-sm text-gray-600 dark:text-gray-400`}>
          ...
          <Text style={tw`font-bold`}>{PERFORMANCE_LEVEL.toUpperCase()}</Text>
        </Text>
        <Text style={tw`text-sm text-gray-600 dark:text-gray-400`}>
      ...
      {results && (
        <View style={tw`bg-white dark:bg-gray-800 p-4 rounded-lg`}>
          ...
          <Text style={tw`text-sm mb-1`}>
            Average FPS:{' '}
            <Text
              style={tw`font-mono ${results.fps < 30 ? 'text-red-500' : results.fps < 50 ? 'text-yellow-500' : 'text-green-500'}`}
            >
              {results.fps}
            </Text>
          </Text>
          <Text style={tw`text-sm mb-1`}>
            Dropped Frames:{' '}
            <Text
              style={tw`font-mono ${results.dropFrames > 50 ? 'text-red-500' : results.dropFrames > 20 ? 'text-yellow-500' : 'text-green-500'}`}
            >
              {results.dropFrames}
            </Text>
          </Text>
          <Text style={tw`text-sm`}>
            Avg Render Time:{' '}
            <Text
              style={tw`font-mono ${results.avgRenderTime > 16.67 ? 'text-red-500' : results.avgRenderTime > 8 ? 'text-yellow-500' : 'text-green-500'}`}
            >
