...
import { getTheme, baseColors, darkTheme, lightTheme } from '../constants/Colors';
...
const MemoizedIcon = memo<{
  ...
  height: number;
  color: string;
}>(({ Icon, width, height, color }) => <Icon width={width} height={height} color={color} />);
...
const componentStyles = StyleSheet.create({
  ...
  locationTrackingContainer: {
    marginHorizontal: CONTAINER_PADDING.horizontal + 30,
  },
  pillButtonsWrapper: {
    // transparent background - no need for explicit color
    borderRadius: 45,
    padding: 4,
  },
  pillButtonsRow: {
  ...
  ({
    ...
    return (
      <Pressable
        onPressIn={onPress}
        onLayout={onLayout}
        style={isHomeLayout ? componentStyles.homeLayoutIcon : baseStyles.tabItem}
...
const StartStopButton = memo<{
  ...
  ({ isRunning, onPress, theme, isDark }) => {
    ...
    const textColor = isDark
      ? isRunning
        ? theme.textPrimary
        : theme.background // Dark mode: white for stop, black for start
    ...
    return (
      <Pressable
        onPressIn={onPress}
        style={[
          componentStyles.startStopButton,
          {
            backgroundColor,
        ...
        android_disableSound
      >
        <View style={componentStyles.startStopButtonContainer}>
          <Text style={[componentStyles.startStopButtonText, { color: textColor }]}>
            {isRunning ? 'Stop' : 'Start'}
          </Text>
          {isRunning ? (
            <StopCircleIconSolid width={12} height={12} color={textColor} />
          ) : (
            <BoltIconSolid width={16} height={16} color={textColor} />
          )}
        </View>
      </Pressable>
    );
  },
  (prev, next) => prev.isRunning === next.isRunning && prev.isDark === next.isDark,
);
...
export const CustomTabBar = memo<BottomTabBarProps>(
  ({ state, navigation }) => {
    ...
    const config =
      currentRoute && currentRoute in NAV_CONFIG
        ? NAV_CONFIG[currentRoute as keyof typeof NAV_CONFIG]
        : DEFAULT_CONFIG;
      ...
      return {
        styles: {
          tabBar: tw.style('overflow-hidden', {
            backgroundColor: isDark ? darkTheme.surface : lightTheme.backgroundSecondary,
          }),
    ...
    const getInitialHeight = () => {
      const initialExpanded = config.lockExpanded;
    ...
    React.useEffect(() => {
      const shouldExpand = config.lockExpanded;
      ...
      if (config.lockExpanded && !isExpanded) {
        setIsExpanded(true);
      } else if (!config.lockExpanded && isExpanded && currentRoute !== 'index') {
        setIsExpanded(false);
      }
    ...
    const renderNavigationRow = useCallback(() => {
        ...
        return (
          <View
            style={[
              ...
              { height: ROW_HEIGHT, justifyContent: 'space-evenly' as const },
            ]}
          >
      ...
      return (
        <View
          style={[baseStyles.tabsContainer, componentStyles.rowContainer, { height: ROW_HEIGHT }]}
        >
          <Pressable
            ...
            style={componentStyles.homeLayoutIcon}
            android_disableSound
          >
            <MotiView
              animate={{
    ...
    const renderTimerRow = useCallback(() => {
      return (
        <View style={componentStyles.timerRowContainer}>
          <Timer isCompact />
        </View>
      );
    }, []);
    ...
    const renderMiddleControlsRow = useCallback(() => {
      return (
        <View
          style={[
            baseStyles.tabsContainer,
            componentStyles.middleControlsRowContainer,
            { height: ROW_HEIGHT },
          ]}
        >
    ...
    const buttonBaseStyle = useMemo(
      () => ({
        height: ROW_HEIGHT - 4,
        backgroundColor: isDark ? darkTheme.surfaceSecondary : baseColors.gray200,
    ...
    return (
      <View style={baseStyles.container}>
        ...
        {currentRoute === 'index' && (
          <View style={componentStyles.locationTrackingContainer}>
            <View style={componentStyles.pillButtonsWrapper}>
              <View style={componentStyles.pillButtonsRow}>
                ...
                >
                  <Pressable
                    ...
                    accessibilityLabel={
                      isLocationActive ? 'Location tracking active' : 'Enable location tracking'
                    }
                  ...
                  >
                    ...
                    >
                      <Text style={buttonTextStyle}>Location</Text>
                    </MotiView>
                  </Pressable>
                </MotiView>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Text style={buttonTextStyle}>Assistant</Text>
                    </MotiView>
                  </Pressable>
                </MotiView>
              </View>
        ...
        <View
          style={[
            { marginHorizontal: CONTAINER_PADDING.horizontal, marginBottom: insets.bottom },
            componentStyles.navBarWrapper,
          ]}
        >
          <Animated.View
            style={[
              styles.tabBar,
              animatedTabBarStyle,
              componentStyles.tabBarBorder,
              { borderColor: theme.borderSubtle },
            ]}
          >
            <View style={componentStyles.tabBarPadding}>
              {/* Render visible rows */}
              {visibleRows.map((row, index) => (
                <View key={row.id} style={index > 0 ? { marginTop: ROW_GAP } : undefined}>
                  {renderRow(row)}
                </View>
              ))}
            </View>
            ...
            {config.showArrow ? (
              <Pressable
                style={[baseStyles.arrowButton, componentStyles.arrowButtonPosition]}
                onPress={toggleExpansion}
              >
                {isExpanded ? (
                  <ChevronDownIcon width={20} height={20} color={inactiveColor} />
                ) : (
                  <ChevronUpIcon width={20} height={20} color={inactiveColor} />
                )}
              </Pressable>
            ) : null}
          </Animated.View>
  ...
  (prev, next) =>
    prev.state.index === next.state.index && prev.state.routes.length === next.state.routes.length,
);
